...
class QDeviceIO(QtCore.QObject):
    ...
    class Worker_DAQ(QtCore.QObject):
        ...
        """
        _signal_worker_DAQ_stopped = QtCore.pyqtSignal()
        ...
        def _do_work(self):  
            if self.DEBUG:
                tprint("Worker_DAQ  %s: work @ thread %s" %
                       (self.dev.name, cur_thread_name()), self.DEBUG_color)
        ...
        def pause(self):
            ...
            if self._trigger_by == DAQ_trigger.CONTINUOUS:
                ...
                if self.DEBUG:
                    tprint("Worker_DAQ  %s: request pause" % 
                           self.dev.name, self.DEBUG_color)
        ...
        def unpause(self):
            ...
            if self._trigger_by == DAQ_trigger.CONTINUOUS:
                ...
                if self.DEBUG:
                    tprint("Worker_DAQ  %s: request unpause" % 
                           self.dev.name, self.DEBUG_color)
    ...
    class Worker_send(QtCore.QObject):
        ...
        def _do_work(self):
            if self.DEBUG:
                tprint("Worker_send %s: work @ thread %s" %
                       (self.dev.name, cur_thread_name()), self.DEBUG_color)
            ...
            if self.DEBUG:
                tprint("Worker_send %s: done running" % 
                       self.dev.name, self.DEBUG_color)
